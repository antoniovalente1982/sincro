...
async function main() {
    ...
    for (const title of sheets.slice(0, 10)) {
        const rowRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${conn.credentials.spreadsheet_id}/values/${encodeURIComponent(title)}!A1:Z5`, { headers: { 'Authorization': `Bearer ${token}` }})
        const rowData = await rowRes.json()
        if (rowData.values && rowData.values.length > 0) {
             console.log(`\nTAB: "${title}" => Rows: ${rowData.values.length}, Header:`, rowData.values[0])
        }
    }
}
main()
