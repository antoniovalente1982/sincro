...
async function main() {
    ...
    for (const title of sheetTitles) {
        const rowRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${conn.credentials.spreadsheet_id}/values/${encodeURIComponent(title)}!A:Z`, { headers: { 'Authorization': `Bearer ${token}` }})
        const rowData = await rowRes.json()
        if (rowData.values && rowData.values.length > 1) {
             console.log(`\nTAB: "${title}" => Total rows: ${rowData.values.length}. Last row:`, rowData.values[rowData.values.length-1])
        }
    }
}
main()
