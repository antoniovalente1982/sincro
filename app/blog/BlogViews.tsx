...
export function BlogArticle({ post, consultationHref, preview = false, related = [], search = '' }: { post: BlogPost; consultationHref: string; preview?: boolean; related?: BlogPost[]; search?: string }) {
    ...
    const female = post.landingTheme === 'femminile' || post.topic === 'femminile'
    const child = female ? 'tua figlia' : 'tuo figlio'
    const help = female ? 'aiutarla' : 'aiutarlo'
    return <AdvertorialFrame date={post.status === 'active' && date ? date : 'Bozza in revisione'} preview={preview}
        note={{ title: 'Da dove partire.', text: `Raccontaci una situazione concreta. Valutiamo insieme se il percorso è adatto a ${child}.`, href: '#consulenza', label: 'Il primo confronto' }}>
        ...
        {sections.map((section, index) => <Fragment key={index}>
            ...
            {sections.length > 1 && index === ctaAfter && <div className={styles.inlineCta}>
                <p><strong>Hai già provato ad {help}. Da dove puoi ripartire?</strong><br />Porta un episodio al primo confronto: mettiamo a fuoco i tuoi dubbi e valutiamo se il percorso è adatto alla vostra situazione.</p>
                <ConsultationLink href={consultationHref} text={`Voglio capire come ${help}`} />
        ...
        <section className={styles.consultation} id="consulenza" aria-labelledby="consulenza-titolo">
            <h2 id="consulenza-titolo">Hai riconosciuto la situazione di {child}?</h2>
            ...
            <p>Il primo confronto gratuito è rivolto a te, il genitore che sta valutando come {help}. Partiamo da ciò che hai osservato e dai tentativi già fatti, per capire quali domande approfondire e se Metodo Sincro può essere pertinente.</p>
            <p>Se emerge una proposta, chiariremo come coinvolgere {child}, che cosa prevede il lavoro e quale investimento richiede.</p>
            <ConsultationLink href={consultationHref} text={`Voglio capire come ${help}`} />
