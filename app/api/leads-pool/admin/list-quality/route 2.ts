...
export async function GET() {
    ...
    const [{ data: lists }, { data: pool }, { data: calls }] = await Promise.all([
        admin.from('lead_lists').select('id, name, source_format, total_count').eq('organization_id', orgId),
        admin.from('lead_pool').select('list_id, status, feedback, call_count').eq('organization_id', orgId),
        admin.from('lead_calls').select('started_at, connected_at, duration_seconds').eq('organization_id', orgId),
    ])
    ...
    return NextResponse.json({ success: true, lists: listQuality, best_hours: bestHours, total_calls: (calls || []).length })
}
